...
class ViewTournament:
    ...
    def __init__(self, stdscr):
        ...
        # some dimensions based on the dimension of the terminal used for
        # drawing windows and pads
        self.outer_wind_height, self.outer_wind_width = (
            stdscr.getmaxyx()[0], stdscr.getmaxyx()[1] - 2)
        self.left_wind_width = (self.outer_wind_width - 3) * 2 // 3
        self.right_wind_width = (
                self.outer_wind_width - self.left_wind_width - 2)
    ...
    def draw_static_elements(self,
        ...
        self.info_wind = self.create_information_wind(name, place, date_start,
    ...
    def initialize_unstarted(self, list_available_players,
                             list_participants_score):
    ...
    def create_outer_wind(self, title="{{default_title}}"):
        """
        Create the outer window that will be used as a parent by others
        windows.
        """
        outer_wind = curses.newwin(self.outer_wind_height,
                                   self.outer_wind_width,
                                   0,
                                   1)
        outer_wind.box()
        box_title = f" {title} "
        outer_wind.addstr(0,
                          (self.outer_wind_width - len(box_title)) // 2,
                          box_title)
    ...
    def create_command_wind(self):
        ...
        command_wind = self.outer_wind.derwin(
            8,
            self.right_wind_width,
            1,
            self.left_wind_width + 1)
        command_wind.clear()
    ...
    def create_ranking_wind(self):
        ...
        ranking_wind = self.outer_wind.derwin(self.bottom_wind_height,
                                              self.right_wind_width,
                                              9,
                                              self.left_wind_width + 1)
        ranking_wind.box()
    ...
    def create_content_wind(self):
        ...
        bottom_left_wind = self.outer_wind.derwin(self.bottom_wind_height,
                                                  self.left_wind_width,
                                                  9,
                                                  1)
    ...
    def create_form_wind(self, form_type):
        ...
        form_add_ranking_wind = self.content_wind.derwin(
            3,
            len(data_map[form_type][0]) + 6,
            (self.content_wind.getmaxyx()[0] - 3) // 2,
            (self.content_wind.getmaxyx()[1] - len(data_map[form_type][0]) - 6)
            // 2)
        ...
        form_add_ranking_wind.addstr(
            1,
            form_width - len(data_map[form_type][1]),
            data_map[form_type][1])
    ...
    def create_match_wind(self, match_data):
        ...
        self.match_wind = self.content_wind.derwin(
            6,
            content_wind_w - 8,
            (content_wind_h - 6) // 2,
            2)
        ...
        match_string = self.reformat_name_vs_name(match_data['left'],
                                                  match_data['right'],
                                                  target_length)
        self.match_wind.addstr(2, 3, match_string)
        self.match_wind.addstr(
            3,
            (target_length + 8 - len(match_data['status'])) // 2,
            match_data['status'])
        self.match_wind.refresh()

    def update_command_wind(self, command_key='command_tournament'):
        """
        Use the mapping of commands in the attributes of the class to update
        command window.
        """
    ...
    def update_ranking_pad(self, list_tuple_player_score=None):
        """
        Create a pad in the ranking window to allow the user to scroll down
        and up if needed.
        """
        ...
        self.ranking_pad = curses.newpad(new_pad_height,
                                         self.right_wind_width - 6)
        ...
        self.ranking_pad.refresh(0, 0,
                                 ...
                                 self.outer_wind_height - 3,
                                 self.outer_wind_width - 3)
    ...
    def update_content_pad_players(self, list_dict_id_player):
        """
        Fill the content pad with the list of players that are not already
        participants of the Tournament.
        """
        pad_height = len(list_dict_id_player) + 1
        self.content_pad_players = curses.newpad(pad_height,
                                                 self.left_wind_width - 6)
    ...
    def update_content_pad_tournament(self, list_dict_rounds):
        ...
        self.content_pad_tournament = curses.newpad(pad_height,
                                                    self.left_wind_width - 6)
        self.content_pad_tournament.clear()
    ...
    def update_content_wind_round(self, round_name):
        ...
        headers = self.reformat_id_match_status(
            {'id': ' #ID',
    ...
    def update_content_pad_round(self, round_match_data):
        ...
        self.content_pad_round = curses.newpad(pad_height,
                                               self.left_wind_width - 6)
    ...
    def reformat_name_score(self, player_name_score):
        ...
        separator = ' | '
        target_length = (
                self.right_wind_width - 6 - len(separator) - len('Score'))
        player_string = self.reformat_name([player_name_score['last_name'],
                                           player_name_score['first_name']],
                                           target_length)
    ...
    def reformat_id_player_name(self, id_player_name):
        ...
        player_string = self.reformat_name([id_player_name['last_name'],
                                           id_player_name['first_name']],
                                           target_length)
    ...
    def reformat_id_round_status(self, id_round_status):
        ...
        separator = ' | '
        target_length = (
                self.left_wind_width - 6 - 2 * len(separator) - len('#000')
                - len('not started'))
    ...
    def reformat_name_vs_name(self, player_left, player_right, target_length):
        """ Utility method to clean data before display """
        match_string_left = self.reformat_name([player_left['last_name'],
                                               player_left['first_name']],
                                               target_length // 2, 'right')
        match_string_right = self.reformat_name([player_right['last_name'],
                                                player_right['first_name']],
                                                target_length // 2)
    ...
    def reformat_id_match_status(self, id_match_status):
        ...
        target_length = (
                self.left_wind_width - 6 - 2 * len(separator) - len('#000')
                - len('pending...') - len(" vs "))
        match_string = self.reformat_name_vs_name(id_match_status['left'],
                                                  id_match_status['right'],
                                                  target_length)
        id_string = str(id_match_status['id']).rjust(len('#000'))
        status_string = (
            str(id_match_status['match_status']).ljust(len('pending...')))
    ...
    def move_pad_down(self):
        ...
        current_pad_height = self.current_pad.getmaxyx()[0]
        if (self.pad_start_line <
                current_pad_height - (self.bottom_wind_height - 5)):
            self.pad_start_line += 1

    def add_player(self):
        """
        Method following a user input, send request to add a player to
        controller.
        """
    ...
    def begin_tournament():
        """
        Method following a user input, send request to begin tournament to
        controller.
        """
    ...
    def ranking_menu():
        """
        Method following a user input, send request to start the ranking_menu
        to controller.
        """
        return 'RANKING'
    ...
    def quit():
        """
        Method following a user input, send request to leave current menu
        to controller.
        """
        return 'EXIT'
    ...
    def sort_score(self):
        """
        Method following a user input, send request to sort the ranking
        display based on score to controller.
        """
    ...
    def sort_name(self):
        """
        Method following a user input, send request to sort the ranking
        display based on name to controller.
        """
        return 'SORT', self.SORT_FIELDS[0], False

    def select_round(self):
        """
        Method following a user input, send request to select a specific round
        to controller.
        """
    ...
    def select_match(self):
        """
        Method following a user input, send request to select a specific match
        to controller.
        """
    ...
    def complete_round():
        """
        Method following a user input, send request to mark current round as
        complete to controller.
        """
    ...
    def win_left():
        """
        Method following a user input, send request to update match result to
        controller.
        """
    ...
    def start_new_view(self):
        """
        View displaying non-started tournament, wait for specific input to
    ...
    def start_tournament_view(self):
        """
        View displaying started tournament, wait for specific input to
        transmit to controller.
        """
        local_key_action = {
            curses.KEY_DOWN: self.key_action['move_down'],
    ...
    def start_ranking_menu(self):
        """
        View modifying ranking window, wait for specific input to transmit to
        controller.
        """
        local_key_action = {
            curses.KEY_DOWN: self.key_action['move_down'],
    ...
    def start_match_view(self, match_data):
        """
        View displaying a window of a selected match, wait for specific input
        to transmit to controller.
        """
        local_key_action = {
