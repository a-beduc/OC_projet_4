...
class Round(_BaseModel):
    def __init__(self,
        ...
        :param matches_pairs: A set composed of pairs of players_id
        {('p_1', 'p_2'),('p_3','p_4')} that will be used to create matches of
        the rounds.
         2024/09/14 : As of today, the pair are generated by an instance of
         pairing model
    ...
    def _create_instance_from_json(cls,
        ...
        :param item_data:
        :param round_id:
        :param save_to_db: must be false to avoid copy of round instance in
        database
        ...
        instance.matches = [Match.from_json(match_id) for match_id
                            in item_data["matches"]]
        return instance
    ...
    def create_matches(self):
        ...
        :return:
        """
        self.matches.extend([Match(pair[0], pair[1]) for pair
                             in self.matches_pairs])

    def end_round(self):
        """
        End the round if every match is finished
        Save the state of the round and the time it was completed in the
        database.
        """
        if not self.is_finished:
            if any(not match.is_finished for match in self.matches):
                raise ValueError("One or more matches are not finished.")
    ...
    def get_match_by_player_id(self, player_id: str):
        ...
        :return: Match object
        """
        return next((match for match in self.matches if player_id
                     in match.score.keys()), None)
