...
class ViewTableTournaments(ViewTableBase):
    """
    A class for displaying and managing a table of tournaments using the
    curses library.
    Inherits from ViewTableBase and implements specific behavior for
    tournament data.

    Constants:
        COMMAND (list): Instructions for navigating the table, creating new
        tournaments, and loading existing ones.
        SORTS_FIELDS (list): Fields by which the tournament table can be
        sorted.
    """
    ...
    def create_content(self, data=None):
        """
        Creates the table content for tournament as string lines, either
        ...
        fixed_length = (
                len(software_id_header) + len(date_start_header)
                + len(date_end_header) + len(number_participants_header)
                + len(number_rounds_header) + len(completed_header)
                + (7 * len(separator)))
        ...
        if data is None:
            ...
            return separator.join([software_id_header, name_header,
                                   place_header, date_start_header,
                                   date_end_header, number_participants_header,
                                   number_rounds_header, completed_header])
        else:
            ...
            return separator.join([software_id, name, place, date_start,
                                   date_end, number_participants,
                                   number_rounds, complete])
    ...
    def get_header_index(header_string):
        ...
        return (idx_id, idx_name, idx_place, idx_date_start, idx_date_end,
                idx_participants, idx_rounds, idx_complete)
    ...
    def create_load_tournament_wind(self):
        """
        Creates a window to prompt the user to enter the ID of a tournament
        to load.
        """
        clearing_space_height, clearing_space_width = (7, 26)
        clearing_wind = self.outer_wind.derwin(
            clearing_space_height,
            clearing_space_width,
            (self.outer_height - clearing_space_height) // 2,
            (self.outer_width - clearing_space_width) // 2)
        clearing_wind.clear()
        clearing_wind.refresh()

        load_tournament_height, load_tournament_width = (
            clearing_space_height - 2,
            clearing_space_width - 2)
        load_tournament_wind = clearing_wind.derwin(
            load_tournament_height,
            load_tournament_width,
            1,
            1)
        load_tournament_wind.clear()
        load_tournament_wind.box()
        load_title = " Load Tournament "
        load_tournament_wind.addstr(
            0,
            (load_tournament_width - len(load_title)) // 2,
            load_title)
        load_text = "ID : "
        middle_width_of_tournament_wind = (
                load_tournament_wind.getmaxyx()[1] // 2)
        load_tournament_wind.addstr(
            2,
            middle_width_of_tournament_wind - len(load_text),
            load_text)
        load_tournament_wind.refresh()
        input_win = load_tournament_wind.derwin(
            1,
            5,
            2,
            middle_width_of_tournament_wind)
